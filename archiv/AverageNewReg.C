...
void AverageNewReg(){
  ...
  for(int islug=1;islug<=94;islug++){
    ...
    mini_tree->AddFriend(minireg_tree);
    if(mini_tree==NULL){
    ...
    Int_t run_number=0;
    Int_t mini_number=0;
    Double_t fMini_number=0;
    Int_t last_run_number=0;
    mini_tree->SetBranchAddress("run",&run_number);
    mini_tree->SetBranchAddress("mini",&fMini_number);
    ...
    for(int ievt=0;ievt<nEntries;ievt++){
      ...
      mini_number = (Int_t)fMini_number;
  ...
  TFile *output_rf =  TFile::Open("prex_grand_average_regress_newReg.root","RECREATE");
  ...
  TaResult fSlugLog_md("average_by_slug_maindet_newReg.log");
  TaResult fSlugLog_beamline("average_by_slug_beamline_newReg.log");
  ...
  while(iter_slug!=fSlugStatBuilderMap.end()){
    ...
    (*iter_slug).second.PullFitAllChannels("./plots/newReg_slug"+slug_label+".pdf");
  ...
  while(iter_pitts!=fPittsStatBuilderMap.end()){
    ...
    (*iter_pitts).second.PullFitAllChannels(Form("./plots/newReg_pitts%d.pdf",fPittsID));
  ...
  fPittsStatBuilder.PullFitAllChannels("newReg_pitts_pullfit.pdf");
  fPittsNullStatBuilder.PullFitAllChannels("newReg_pitts_null_pullfit.pdf");
  TaResult fPittLog_md("average_by_pitt_maindet_newReg.log");
  TaResult fPittLog_mdnull("null_by_pitt_maindet_newReg.log");
  TaResult fPittLog_beamline("average_by_pitt_beamline_newReg.log");
  ...
  while(iter_wien!=fWienStatBuilderMap.end()){
    ...
    (*iter_wien).second.PullFitAllChannels(Form("./plots/newReg_wien%d.pdf",fWienID));
  ...
  fWienStatBuilder.PullFitAllChannels("newReg_wien_pullfit.pdf");
  TaResult fWienLog_md("average_by_wien_maindet_newReg.log");
  TaResult fWienLog_beamline("average_by_wien_beamline_newReg.log");
