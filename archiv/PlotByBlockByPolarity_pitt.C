...
void PlotByBlockByPolarity_pitt(){
  gStyle->SetOptFit(1);
  ...
  TTree *neg_tree = (TTree*)neg_file->Get("pitt");
  TTree *pos_tree = (TTree*)pos_file->Get("pitt");
  TTree *neutral_tree = (TTree*)neutral_file->Get("pitt");
  
  TFile* normal_file = TFile::Open("prex_grand_average_norm.root");
  TTree *normal_tree = (TTree*)normal_file->Get("pitt");
  ...
  gPad->SetBottomMargin(0.05);
  c1->cd();
  c1->Print("prex_pitt_average_byblock.pdf[");
  for(int idet=0;idet<nDet;idet++){
    ...
    TString draw_cmd = Form("%s*%f:%s.error*%f:pitt",
    ...
    Int_t nData = pos_tree->Draw(draw_cmd,channel_cut,"goff");
    if(nData==0)
      continue;
    
    c_norm->cd();
    c_norm->Clear("D");
    normal_tree->Draw(draw_cmd,channel_cut,"goff");
    ...
    ger_norm->Fit("pol0","Q");
    ger_norm->SetTitle(device_name+unit+";pitt;"+unit);
    
    c_norm->Print("prex_pitt_average_byblock.pdf");
    ...
    neg_tree->Draw(draw_cmd,channel_cut,"goff");
    ...
    mg->Add(ger_neg,"LP");
    mg->Draw("A");
    mg->SetTitle(device_name+unit+";pitt;"+unit);
    ...
    neutral_tree->Draw(draw_cmd,channel_cut,"goff");
    ...
    ger_neutral->GetXaxis()->SetLimits(0,25);
    ...
    c_pol->Print("prex_pitt_average_byblock.pdf");
    ...
    c2->Print("prex_pitt_average_byblock.pdf");
    ...
    for(int iblk=0;iblk<4;iblk++){
      ...
      ger_block->SetLineStyle(2);
      ger_block->Draw("ALP");
      ger_block->SetTitle(device_name+Form(".block%d Neutral Pick-up",iblk)+unit+";pitt;"+unit);
