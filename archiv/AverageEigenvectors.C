...
void AverageEigenvectors(){
  ...
  for(int islug=1;islug<=94;islug++){
    ...
    eig_tree->SetBranchAddress("mini",&mini_id);
  ...
  while(iter_slug!=fSlugStatBuilderMap.end()){
    ...
    (*iter_slug).second.PullFitAllChannels("./plots/evector_"+slug_label+".pdf");
  ...
  while(iter_pitts!=fPittsStatBuilderMap.end()){
    ...
    (*iter_pitts).second.PullFitAllChannels(Form("./plots/evector_Pitt%d.pdf",fPittsID));
  ...
  while(iter_wien!=fWienStatBuilderMap.end()){
    ...
    (*iter_wien).second.PullFitAllChannels(Form("./plots/evector_Wien_%s.pdf",
