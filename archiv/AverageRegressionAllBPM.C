...
void AverageRegressionAllBPM(){
  ...
  for(int islug=1;islug<=94;islug++){
    ...
    TTree* mini_tree = (TTree*)input_file->Get("mini_regall");
    if(mini_tree==NULL){
  ...
  TFile *output_rf =  TFile::Open("prex_grand_average_reg_allbpm.root","RECREATE");
  ...
  TaResult fSlugLog_md("average_by_slug_maindet_reg_allbpm.log");
  ...
  while(iter_slug!=fSlugStatBuilderMap.end()){
    ...
    (*iter_slug).second.PullFitAllChannels("./plots/reg_allbpm_slug"+slug_label+".pdf");
  ...
  while(iter_pitts!=fPittsStatBuilderMap.end()){
    ...
    (*iter_pitts).second.PullFitAllChannels(Form("./plots/reg_allbpm_pitts%d.pdf",fPittsID));
  ...
  fPittsStatBuilder.PullFitAllChannels("reg_allbpm_pitts_pullfit.pdf");
  fPittsNullStatBuilder.PullFitAllChannels("reg_allbpm_pitts_null_pullfit.pdf");
  TaResult fPittLog_md("average_by_pitt_maindet_reg_allbpm.log");
  TaResult fPittLog_mdnull("null_by_pitt_maindet_reg_allbpm.log");
  ...
  while(iter_wien!=fWienStatBuilderMap.end()){
    ...
    (*iter_wien).second.PullFitAllChannels(Form("./plots/reg_allbpm_wien%d.pdf",fWienID));
  ...
  fWienStatBuilder.PullFitAllChannels("reg_allbpm_wien_pullfit.pdf");
  TaResult fWienLog_md("average_by_wien_maindet_reg_allbpm.log");
