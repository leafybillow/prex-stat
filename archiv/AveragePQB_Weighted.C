...
void AveragePQB_Weighted(){
  ...
  vector<TString> fBPMNameList={"diff_bpmE",
				"diff_bpm4aX","diff_bpm4eX",
				"diff_bpm4aY","diff_bpm4eY"};  
  vector<TString> fBCMNameList={"asym_bcm_target"};
  vector<TString> fDetNameList={"lagr_asym_us_avg","lagr_asym_usr","lagr_asym_usl"};
  ...
  fDeviceNameList.insert(fDeviceNameList.end(),
  			 fDetNameList.begin(),fDetNameList.end());
  ...
  for(int islug=1;islug<=94;islug++){
    ...
    burst_tree->AddFriend( (TTree*)input_file->Get("burst_mulc") );
    if(burst_tree==NULL){
      cerr << " Error: Burst Tree not found in "
	   << file_name << endl;
      continue;
    }
    TFile* lagr_file = TFile::Open(Form("./rootfiles/MergedLagrange_slug%d.root",islug));
    TTree* mini_lagr  = (TTree*)lagr_file->Get("mini_lagrall");
    burst_tree->AddFriend(mini_lagr);
    Int_t nEntries = burst_tree->GetEntries();
    Int_t run_number=0;
    Int_t mini_id=0;
    Int_t last_run_number=0;
    burst_tree->SetBranchAddress("run",&run_number);
    burst_tree->SetBranchAddress("minirun",&mini_id);
  ...
  TFile *output_rf =  TFile::Open("prex_grand_average_PQB_respin2_weighted.root","RECREATE");
  ...
  TaResult fSlugLog_beamline("average_by_slug_beamline_weighted.log");
  ...
  while(iter_slug!=fSlugStatBuilderMap.end()){
    ...
    (*iter_slug).second.PullFitAllChannels("./plots/pqb_weighted_slug"+slug_label+".pdf");
  ...
  while(iter_pitts!=fPittsStatBuilderMap.end()){
    ...
    (*iter_pitts).second.PullFitAllChannels(Form("./plots/pqb_weighted_pitts%d.pdf",fPittsID));
  ...
  fPittsStatBuilder.PullFitAllChannels("pqb_weighted_pitts_pullfit.pdf");
  fPittsNullStatBuilder.PullFitAllChannels("pqb_weighted_pitts_null_pullfit.pdf");

  TaResult fPittLog_beamline("average_by_pitt_beamline_weighted.log");
  ...
  while(iter_wien!=fWienStatBuilderMap.end()){
    ...
    cout << fWienID << endl;
    fWienStatBuilder.SetLabel(Form("Wien %d",fWienID));
    (*iter_wien).second.PullFitAllChannels(Form("./plots/pqb_weighted_wien%d.pdf",fWienID));
  ...
  TaResult fWienLog_beamline("average_by_wien_beamline_weighted.log"); 
  ReportBeamLineLogByIHWP(fWienStatBuilder,fWienLog_beamline);
  fWienStatBuilder.PullFitAllChannels("pqb_weighted_wien_pullfit.pdf");
