...
void AverageMul(){
  ...
  for(int islug=0;islug<=94;islug++){
    TString file_name = Form("./rootfiles/MulMerged_slug%d.root",islug);
    TFile* input_file = TFile::Open(file_name);
    if(input_file==NULL){
      cerr <<  file_name << " is not found and is skipped" << endl;
      continue;
    }
    TTree* muls_tree = (TTree*)input_file->Get("muls");
    if(muls_tree==NULL){
      cerr << " Error: Dither Tree not found in "
	   << file_name << endl;
      continue;
    }
    Int_t nEntries = muls_tree->GetEntries();
    Int_t run_number=0;
    Int_t last_run_number=0;
    muls_tree->SetBranchAddress("run",&run_number);
    RegisterBranchesAddress(muls_tree,fDeviceNameList,fChannelMap);
    TaRunInfo myRunInfo;
    SLUG_ARM myKey;
    TString detName, bcmName;
    for(int ievt=0;ievt<nEntries;ievt++){
      muls_tree->GetEntry(ievt);
  ...
  TFile *output_rf =  TFile::Open("prex_grand_average_muls.root","RECREATE");
  ...
  TaResult fSlugLog_md("average_by_slug_maindet_mul.log");
  ...
  while(iter_slug!=fSlugStatBuilderMap.end()){
    ...
    (*iter_slug).second.PullFitAllChannels("./plots/mul_slug"+slug_label+".pdf");
  ...
  while(iter_pitts!=fPittsStatBuilderMap.end()){
    ...
    (*iter_pitts).second.PullFitAllChannels(Form("./plots/mul_pitts%d.pdf",fPittsID));
  ...
  fPittsStatBuilder.PullFitAllChannels("mul_pitts_pullfit.pdf");
  TaResult fPittLog_md("average_by_pitt_maindet_mul.log");
  ...
  while(iter_wien!=fWienStatBuilderMap.end()){
    ...
    (*iter_wien).second.PullFitAllChannels(Form("./plots/mul_wien%d.pdf",fWienID));
  ...
  fWienStatBuilder.PullFitAllChannels("mul_wien_pullfit.pdf");
  TaResult fWienLog_md("average_by_wien_maindet_mul.log");
