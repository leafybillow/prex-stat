...
void GetAverageBySlug_dither(){
  ...
  TString det_array[]={"dit_asym_us_avg","dit_asym_usr","dit_asym_usl"};
  
  TString rawdet_array[]={"asym_us_avg","asym_usr","asym_usl"};

  Int_t nDet = sizeof(det_array)/sizeof(*det_array);
  
  TFile* output = TFile::Open("./rootfiles/prex_grand_average_dither.root",
			      "RECREATE");
  ...
  TaResult *fReport_main = new TaResult("output/averages_by_slug_maindet_dither.log");
  TaResult *fReport_beam = new TaResult("output/averages_by_slug_beamline_dither.log");
  ...
  while(iter_slug!=fSlugMap.end()){
    ...
    TFile* input_file = TFile::Open(Form("./rootfiles/dither/prex_ditsum_slug%d.root",mySlug));
    if(input_file==NULL){
    ...
    if(dits_tree==NULL){
      cout << " Error: dit Tree not found in "
    ...
    Bool_t kMatch=kTRUE;
    Double_t run_number;
    dits_tree->SetBranchAddress("run_number",&run_number);

    for(int idet=0;idet<nDet;idet++){
      dits_tree->SetBranchAddress(det_array[idet]+"_mean",
				  &(fDetectorArray[idet]).mean);
      dits_tree->SetBranchAddress(det_array[idet]+"_mean_error",
				  &(fDetectorArray[idet]).error);
      dits_tree->SetBranchAddress(det_array[idet]+"_rms",
				  &(fDetectorArray[idet]).rms);

      dits_tree->SetBranchAddress(rawdet_array[idet]+"_mean",
				  &(fDetectorRawArray[idet]).mean);
      dits_tree->SetBranchAddress(rawdet_array[idet]+"_mean_error",
				  &(fDetectorRawArray[idet]).error);
      dits_tree->SetBranchAddress(rawdet_array[idet]+"_rms",
				  &(fDetectorRawArray[idet]).rms);
    }


    // for(int ibcm=0;ibcm<nBCM;ibcm++){
    //   dits_tree->SetBranchAddress(bcm_array[ibcm]+"_mean",
    // 				  &(fBCMArray[ibcm]).mean);
    //   dits_tree->SetBranchAddress(bcm_array[ibcm]+"_mean_err",
    // 				  &(fBCMArray[ibcm]).error);
    //   dits_tree->SetBranchAddress(bcm_array[ibcm]+"_rms",
    // 				  &(fBCMArray[ibcm]).rms)

    //   TBranch* this_branch=dits_tree->GetBranch(bcm_array[ibcm]);
    //   if(this_branch!=NULL){
    ...
    // for(int ibpm=0;ibpm<nBPM;ibpm++){
    //   TBranch* this_branch=dits_tree->GetBranch(bpm_array[ibpm]);
    //   if(this_branch!=NULL){
    ...
    // 	dits_tree->SetBranchAddress(raw_name+"_"+bpm_array[ibpm],
    // 				    &fSlopes[idet][ibpm]);
    //   }
    // }
    Int_t nevt = dits_tree->GetEntries();
	...
	fReport_beam->AddFloatEntry(fDiffBPM[ibpm]*1e6);
      ...
      TString plot_title = Form("dit: %s slug%d pull fit",
