...
void GetAverageByWien_lagrange(Int_t arm_select);
void GetAverageByWien_lagrange(){
  GetAverageByWien_lagrange(-1);
  GetAverageByWien_lagrange(0);
  GetAverageByWien_lagrange(1);
  GetAverageByWien_lagrange(2);
}
...
void GetAverageByWien_lagrange(Int_t arm_select){
  ...
  TFile *inputRF = TFile::Open("../rootfiles/prex_grand_average_lagrange.root");
  ...
  TString outlog_filename = Form("./output/lagrange_averages_by_wien_maindet%s.log",
				 output_label.Data());
