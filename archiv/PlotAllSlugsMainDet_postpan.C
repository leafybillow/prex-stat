...
void PlotAllSlugMainDet_postpan(){
  ...
  for(int iwien=0;iwien<4;iwien++){
    Int_t this_npt = grand_tree->Draw(Form("spin*%s.mean/ppb:error/ppb:slug",
  				      detector.Data()),
  ...
  mge->GetYaxis()->SetRangeUser(y_min,y_max+0.1*(y_max-y_min));
  ...
  // gPad->Update();
  leg->Draw("same");
  c1->SaveAs("sign-corrected-us-avg.pdf");
  tsw.Print();
}
