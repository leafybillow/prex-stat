...
void PlotByBlockByPolarity(){
  ...
  for(int iplot=0;iplot<2;iplot++){
    c1->Print("prex_slug_average_byblock_"+label[iplot]+".pdf[");
	...
	continue;
      ...
      c_norm->Clear("D");
      TMultiGraph mg_all;
      TLegend leg_all(0.9,0.9,0.7,0.7);
      ...
      ger_norm->Fit("pol0","Q");
      mg_all.Add(ger_norm,"P");
      TGraphErrors *ger_out ,*ger_in;
      if(device_name=="Adet"){
	normal_tree->Draw(draw_cmd,sign_cut[iplot]+"&&"+channel_cut+" &&ihwp==\"OUT\" ","goff");
	ger_out = new TGraphErrors(normal_tree->GetSelectedRows(),
						  normal_tree->GetV3(),normal_tree->GetV1(),
						  0,normal_tree->GetV2());
	ger_out->Fit("pol0","Q");
	ger_out->GetFunction("pol0")->SetLineColor(8);
	normal_tree->Draw(draw_cmd,sign_cut[iplot]+"&&"+channel_cut+" &&ihwp==\"IN\" ","goff");
	
	ger_in = new TGraphErrors(normal_tree->GetSelectedRows(),
						normal_tree->GetV3(),normal_tree->GetV1(),
						0,normal_tree->GetV2());
	ger_in->Fit("pol0","Q");
	ger_in->GetFunction("pol0")->SetLineColor(9);
	
	ger_in->SetMarkerColor(9);
	ger_in->SetMarkerStyle(22);
	ger_in->SetMarkerSize(1.5);
	ger_out->SetMarkerColor(8);
	ger_out->SetMarkerStyle(23);
	ger_out->SetMarkerSize(1.5);
	mg_all.Add(ger_in,"P");
	mg_all.Add(ger_out,"P");
	leg_all.AddEntry(ger_in,"IHWP IN");
	leg_all.AddEntry(ger_out,"IHWP OUT");

      }
      mg_all.SetTitle(device_name+unit+";slug;"+unit);
      mg_all.Draw("AP");

      y_max = mg_all.GetYaxis()->GetXmax();
      y_min = mg_all.GetYaxis()->GetXmin();
      mg_all.GetYaxis()->SetRangeUser(y_min,y_max+0.33*(y_max-y_min));
      if(device_name=="Adet"){
	leg_all.Draw("same");
	gPad->Update();
	
	TPaveStats* psall = (TPaveStats*)ger_norm->FindObject("stats");
	psall->SetX1NDC(0.3);
	psall->SetY1NDC(0.85);
	psall->SetX2NDC(0.5);
	psall->SetY2NDC(0.75);

	TPaveStats* psout = (TPaveStats*)ger_out->FindObject("stats");
	psout->SetTextColor(8);
	psout->SetX1NDC(0.1);
	psout->SetY1NDC(0.9);
	psout->SetX2NDC(0.3);
	psout->SetY2NDC(0.8);
	psout->Draw();
	TPaveStats* psin = (TPaveStats*)ger_in->FindObject("stats");
	psin->SetTextColor(9);
	psin->SetX1NDC(0.1);
	psin->SetY1NDC(0.8);
	psin->SetX2NDC(0.3);
	...
	psin->Draw();
      ...
      c_norm->Print("prex_slug_average_byblock_"+label[iplot]+".pdf");
      ...
      c_pol->Print("prex_slug_average_byblock_"+label[iplot]+".pdf");        
      // TMultiGraph* mg_blk = new TMultiGraph();
	...
	fGraphArray[iblk] = ger_block;
      ...
      leg_norm->Draw("same");
      for(int iblk=0;iblk<4;iblk++){
	TF1 *f1 = new TF1("f1","[0]",0,95);
	f1->SetLineColor(fColor[iblk]);
	fGraphArray[iblk]->Fit("f1","QR","",0,95);
	...
	TF1 *f1pos = new TF1("f1pos","[0]",0,95);
	f1pos->SetLineColor(fColor[iblk]);
	TF1 *f1neg = new TF1("f1neg","[0]",0,95);
	f1neg->SetLineColor(fColor[iblk]);
	
	ger_pos_block->Fit("f1pos","QR0","",0,95);
	gPad->Update();
	...
	ger_neg_block->Fit("f1neg","QR0","",0,95);
	gPad->Update();
	...
	leg_block->Draw("same");
      ...
      c2->Print("prex_slug_average_byblock_"+label[iplot]+".pdf");    
      c2->Clear("D");
