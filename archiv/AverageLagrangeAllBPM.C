...
void AverageLagrangeAllBPM(){
  ...
  for(int islug=1;islug<=94;islug++){
    ...
    TTree* mini_tree = (TTree*)input_file->Get("mini_lagrall");
    if(mini_tree==NULL){
  ...
  TFile *output_rf =  TFile::Open("prex_grand_average_lagr_allbpm.root","RECREATE");
  ...
  TaResult fSlugLog_md("average_by_slug_maindet_lagr_allbpm.log");
  ...
  while(iter_slug!=fSlugStatBuilderMap.end()){
    ...
    (*iter_slug).second.PullFitAllChannels("./plots/lagr_allbpm_slug"+slug_label+".pdf");
  ...
  while(iter_pitts!=fPittsStatBuilderMap.end()){
    ...
    (*iter_pitts).second.PullFitAllChannels(Form("./plots/lagr_allbpm_pitts%d.pdf",fPittsID));
  ...
  fPittsStatBuilder.PullFitAllChannels("lagr_allbpm_pitts_pullfit.pdf");
  fPittsNullStatBuilder.PullFitAllChannels("lagr_allbpm_pitts_null_pullfit.pdf");
  TaResult fPittLog_md("average_by_pitt_maindet_lagr_allbpm.log");
  TaResult fPittLog_mdnull("null_by_pitt_maindet_lagr_allbpm.log");
  ...
  while(iter_wien!=fWienStatBuilderMap.end()){
    ...
    (*iter_wien).second.PullFitAllChannels(Form("./plots/lagr_allbpm_wien%d.pdf",fWienID));
  ...
  fWienStatBuilder.PullFitAllChannels("lagr_allbpm_wien_pullfit.pdf");
  TaResult fWienLog_md("average_by_wien_maindet_lagr_allbpm.log");
