...
void AverageDance(){
  ...
  for(int islug=1;islug<=94;islug++){
    TString file_name = Form("/u/home/tao/work/respin2-sandbox/dance/slug_sum/DanceMerged_slug%d.root",islug);
    TFile* input_file = TFile::Open(file_name);
    if(input_file==NULL){
      cerr <<  file_name << " is not found and is skipped" << endl;
      continue;
    }
    TTree* dits_tree = (TTree*)input_file->Get("mini_dit");
    if(dits_tree==NULL){
  ...
  TFile *output_rf =  TFile::Open("prex_grand_average_dance.root","RECREATE");
  ...
  TaResult fSlugLog_md("average_by_slug_maindet_dance.log");
  ...
  while(iter_slug!=fSlugStatBuilderMap.end()){
    ...
    (*iter_slug).second.PullFitAllChannels("./plots/dance_slug"+slug_label+".pdf");
  ...
  while(iter_pitts!=fPittsStatBuilderMap.end()){
    ...
    (*iter_pitts).second.PullFitAllChannels(Form("./plots/dance_pitts%d.pdf",fPittsID));
  ...
  fPittsStatBuilder.PullFitAllChannels("dance_pitts_pullfit.pdf");
  TaResult fPittLog_md("average_by_pitt_maindet_dance.log");
  ...
  while(iter_wien!=fWienStatBuilderMap.end()){
    ...
    (*iter_wien).second.PullFitAllChannels(Form("./plots/dance_wien%d.pdf",fWienID));
  ...
  fWienStatBuilder.PullFitAllChannels("dance_wien_pullfit.pdf");
  TaResult fWienLog_md("average_by_wien_maindet_dance.log");
