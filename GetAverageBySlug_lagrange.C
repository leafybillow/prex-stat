...
void GetAverageBySlug_lagrange(){
  TStopwatch tsw;
  ...
  TString det_array[]={"redi_asym_us_avg","redi_asym_usr","redi_asym_usl"};
  
  TString rawdet_array[]={"asym_us_avg","asym_usr","asym_usl"};

  Int_t nDet = sizeof(det_array)/sizeof(*det_array);
  
  TFile* output = TFile::Open("../rootfiles/prex_grand_average_lagrange.root",
			      "RECREATE");
  ...
  TaResult *fReport_main = new TaResult("output/lagrange_averages_by_slug_maindet.log");
  TaResult *fReport_beam = new TaResult("output/lagrange_averages_by_slug_beamline.log");
  
  vector<TString> header_main{"Slug","Adet(ppb)", "Error","chi2/NDF"};
  vector<TString> header_beam{"Slug",
  ...
  while(iter_slug!=fSlugMap.end()){
    ...
    TFile* input_file = TFile::Open(Form("../rootfiles/lagrange/prex_lagrange_slug%d.root",mySlug));
    if(input_file==NULL){
    ...
    TTree* mini_tree = (TTree*)input_file->Get("redi");
    if(mini_tree==NULL){
    ...
    for(int idet=0;idet<nDet;idet++){
      ...
      det_ptr->GetLeaf("error")->SetAddress(&(fDetectorArray[idet]).error);
      ...
      det_ptr->GetLeaf("error")->SetAddress(&(fDetectorRawArray[idet]).error);
	...
	this_branch->GetLeaf("error")->SetAddress(&(fBPMArray[ibpm].error));
