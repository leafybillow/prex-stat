...
void AverageRegression6BPM(){
  ...
  for(int islug=1;islug<=94;islug++){
    ...
    TTree* mini_tree = (TTree*)input_file->Get("mini_reg6bpm");
    if(mini_tree==NULL){
  ...
  TFile *output_rf =  TFile::Open("prex_grand_average_reg_6bpm.root","RECREATE");
  ...
  TaResult fSlugLog_md("average_by_slug_maindet_reg_6bpm.log");
  ...
  while(iter_slug!=fSlugStatBuilderMap.end()){
    ...
    (*iter_slug).second.PullFitAllChannels("./plots/reg_6bpm_slug"+slug_label+".pdf");
  ...
  while(iter_pitts!=fPittsStatBuilderMap.end()){
    ...
    (*iter_pitts).second.PullFitAllChannels(Form("./plots/reg_6bpm_pitts%d.pdf",fPittsID));
  ...
  fPittsStatBuilder.PullFitAllChannels("reg_6bpm_pitts_pullfit.pdf");
  fPittsNullStatBuilder.PullFitAllChannels("reg_6bpm_pitts_null_pullfit.pdf");
  TaResult fPittLog_md("average_by_pitt_maindet_reg_6bpm.log");
  TaResult fPittLog_mdnull("null_by_pitt_maindet_reg_6bpm.log");
  ...
  while(iter_wien!=fWienStatBuilderMap.end()){
    ...
    (*iter_wien).second.PullFitAllChannels(Form("./plots/reg_6bpm_wien%d.pdf",fWienID));
  ...
  fWienStatBuilder.PullFitAllChannels("reg_6bpm_wien_pullfit.pdf");
  TaResult fWienLog_md("average_by_wien_maindet_reg_6bpm.log");
